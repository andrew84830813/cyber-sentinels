...
Environment (process activity)
        │
        ▼
[Sensing layer]      src/cyber_ignition_agents/sensor/monitor.py
        │            Watches events. Fires on trigger signatures.
        │            No human involved.
        │
        ▼
[Orchestration]      src/cyber_ignition_agents/agent/graph.py
...
cd cyber-ignition-agents

# Install in editable mode (creates cyber-ignition-agents and cyber-ignition-agents-monitor CLI commands)
pip install -e ".[dev]"
...
```bash
cyber-ignition-agents --scenario A        # same as python demo.py --scenario A
cyber-ignition-agents-monitor --dry-run   # same as python run_continuous.py --dry-run
```
...
python demo.py                     # or: cyber-ignition-agents
...
cyber-ignition-agents --scenario A                       # Category 4 — adaptive orchestrator
cyber-ignition-agents --mode rules --scenario A          # Category 3 — rule-based signatures
```
...
**Sensing** (`src/cyber_ignition_agents/sensor/monitor.py`, `src/cyber_ignition_agents/sensor/orchestrator.py`)
...
**Agent** (`src/cyber_ignition_agents/agent/agent.py`)
...
**Memory** (`src/cyber_ignition_agents/memory/store.py`)
...
The simulation hook is clearly marked in `src/cyber_ignition_agents/sensor/monitor.py`:
...
`cyber_ignition_agents_playground.ipynb` provides a section-by-section walkthrough:
...
Edit `src/cyber_ignition_agents/sensor/monitor.py`:
...
1. Add a function to `src/cyber_ignition_agents/simulation/malicious.py` (or `src/cyber_ignition_agents/simulation/normal.py`) that returns `(events_list, scenario_name)`
2. Add it to `SCENARIOS` in `demo.py`
3. If the scenario uses a new technique, add it to `TRIGGER_SIGNATURES` in `src/cyber_ignition_agents/sensor/monitor.py`
...
Edit `src/cyber_ignition_agents/agent/tools.py` → `lookup_mitre_technique()`, and `src/cyber_ignition_agents/agent/prompts.py` → `SYSTEM_PROMPT` (the quick reference block).
...
│
├── notebooks/
│   └── cyber_ignition_agents_playground.ipynb # interactive section-by-section walkthrough
│
└── output/
